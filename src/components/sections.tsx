...
export function DownloadSection({ onDownload, status, progress = 0, received = 0, total = 0 }: { onDownload: DownloadHandler; status: "idle" | "loading" | "complete" | "error"; progress?: number; received?: number; total?: number }) {
  const benefits = ["Free Download", "Windows", "Offline Play", "No Account Required", "Controller Support", "Frequent Updates"];
  const formatSize = (bytes: number) => `${(bytes / 1024 / 1024).toFixed(1)} MB`;
  return <section id="download" className="relative isolate overflow-hidden py-24 sm:py-32"><div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_50%_45%,rgba(94,182,255,0.3),transparent_25%),linear-gradient(135deg,#17223e,#111827_55%,#29184a)]" /><Particles count={26} color="gold" className="-z-0" /><motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={{ visible: { transition: { staggerChildren: 0.1 } } }} className="mx-auto max-w-4xl px-6 text-center sm:px-8"><motion.p variants={reveal} className="text-xs font-bold uppercase tracking-[0.4em] text-[#9ddcff]">Your journey begins now</motion.p><motion.h2 variants={reveal} className="display mt-5 text-4xl text-white sm:text-6xl">Ready to Begin Your Adventure?</motion.h2><motion.div variants={reveal} className="mx-auto mt-8 grid max-w-2xl gap-3 text-left sm:grid-cols-2">{benefits.map((benefit) => <div key={benefit} className="flex items-center gap-3 rounded-xl border border-white/10 bg-black/20 px-5 py-4 text-white/85"><Check className="h-5 w-5 text-[#8dd9ff]" />{benefit}</div>)}</motion.div><motion.p variants={reveal} className="mt-7 text-sm text-white/55">Estimated download: 134 MB</motion.p>{status === "loading" && <motion.div variants={reveal} className="mx-auto mt-5 max-w-md rounded-2xl border border-cyan-200/25 bg-slate-950/45 p-4 text-left"><div className="mb-2 flex justify-between text-xs text-cyan-100"><span>Downloading game files</span><span>{progress}%</span></div><div className="h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-cyan-300 via-blue-400 to-violet-400 transition-[width] duration-200" style={{ width: `${progress}%` }} /></div><div className="mt-2 flex justify-between text-[11px] text-white/55"><span>{formatSize(received)}</span><span>{total ? formatSize(total) : "Preparing…"}</span></div></motion.div>}<motion.div variants={reveal} className="mt-5"><DownloadButton onDownload={onDownload} label={status === "loading" ? `${progress}% Downloading` : status === "complete" ? "Download Complete" : "Download Free (Windows · 134 MB)"} /></motion.div>{status === "complete" && <p className="mt-4 text-sm text-emerald-200">Congratulations! Your download is complete.</p>}{status === "error" && <p className="mt-4 text-sm text-red-300">The download could not be started. Please try again.</p>}</motion.div></section>;
}
